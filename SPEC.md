...
Synchronization verifies provider availability and `WRITE_EXERCISE` permission on every attempt, maps Room sessions from snapshot data, and writes them through one gateway hidden behind a small abstraction. Full-history catch-up loads and submits finalized sessions in reasonable batches when sync is enabled, on application start or resume while enabled, after backup restore, and when the user taps Sync now. Repeated catch-up is duplicate-safe through `clientRecordId` and does not read Health Connect to find existing records.

After the Room finish transaction and progression work succeed, workout completion queues a best-effort asynchronous write for only the newly finalized session and returns without waiting for Health Connect. This post-workout path never runs a full-history catch-up. Health Connect is not part of the Room transaction, and any provider, permission, or transport failure cannot fail or roll back workout completion. Turning sync off stops future automatic writes without deleting external records or changing Room. Restored finalized History may be exported by a later catch-up using the same deterministic identity; Health Connect records themselves are not backed up or deleted during restore.
...
- Post-workout synchronization is a supervised asynchronous write of only the newly finalized session, queued after successful Room finalization so the completion flow never waits for Health Connect. Sync now, enabling synchronization, app resume, and restore retain the full-history batched catch-up path. Both paths use the same mapping and write checks, and external failures never roll back or fail local workout completion.
